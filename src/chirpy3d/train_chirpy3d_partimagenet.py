...
from partcraft.dataset import PartImageNet, MakeSquareTransform
...
def parse_args():
    ...
    parser.add_argument("--filename", default="train_quads.txt")
    parser.add_argument("--mask_filename", default="mask_quads.txt")
...
def collate_fn(args, tokenizer, placeholder_token):
    ...
    train_transforms_mask = transforms.Compose(
        [
    ...
    square_transform = MakeSquareTransform()

    def f(examples):
        raw_images = [
            train_resizecrop(square_transform(example["pixel_values"]))
        ...
        bg_color = lambda x: (
            (255, 255, 255) if x == 4 else (0, 0, 0)
        )  # 4 is partimagenet bg
        masks = [
            [
                train_resizecrop(square_transform(m, bg_color(mi)))
                for mi, m in enumerate(example["masks"])
            ]
            for example in examples
        ]
        masks = [
            torch.stack([train_transforms_mask(m) for m in mask]) for mask in masks
        ]
        masks = torch.stack(masks).squeeze(2).float()  # (N, M, H, W)
        ...
        for i in range(len(examples)):
            ...
            appeared = examples[i]["appeared"]
            if args.skip_background:
                appeared.remove(args.bg_code)
            ...
            not_appeared = [j for j in range(args.num_parts) if j not in appeared]

            for a in not_appeared:
                masks[i, a] = 0
        ...
        image_masks = (masks.sum(dim=1) > 0).float()  # (B, H, W)
...
def main():
    ...
    train_dataset = PartImageNet(
        args.train_data_dir,
        args.filename,
        mask_filename=args.mask_filename,
        bg_code=args.bg_code,
        repeat=args.repeat,
        path_prefix=args.train_data_dir + "/",
        quads_only="quads" in args.filename,
    )
    ...
    # param_group_schedulers = [
    #     {'type': 'linear', 'T_max': 10000},  # linear decay to 0 for param_groups[0]
    #     {'type': 'constant'},  # constant LR for param_groups[1]
    # ]
    # # Create a custom learning rate scheduler
    # lr_scheduler = CustomLRscheduler(optimizer, param_group_schedulers)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet, simple_mapper):
                ...
                # for partiamgenet only!!
                batch["dino_cls"] = simple_mapper.get_dummy_dino_features(
                    batch["codes"].size(0)
                )
