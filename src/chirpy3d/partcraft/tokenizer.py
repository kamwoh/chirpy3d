...
class MultiTokenCLIPTokenizer(CLIPTokenizer):
    ...
    def add_placeholder_tokens(
        self, placeholder_token, *args, num_vec_per_token=1, **kwargs
    ):
        output = []
    ...
    def replace_placeholder_tokens_in_text(
        self, text, vector_shuffle=False, prop_tokens_to_load=1.0
    ):
        """
        ...
        for placeholder_token in self.token_map:
            if placeholder_token in text:
                ...
                text = text.replace(
                    placeholder_token, " ".join(tokens)
                )  # <part>_0 <part>_1 -> <part>
        return text
